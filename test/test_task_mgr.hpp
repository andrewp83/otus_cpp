...
TEST(test_job, base_job) {
    ...
    job.add_task(sum_task);
    job.set_finish_callback([&sum, sum_task](Job*){
    ...
    ASSERT_EQ(sum, 5) << "1 is not equal 0";
}
...
TEST(test_job, base_multi_tasks) {
    ...
    job.add_task(sum_task);
    job.add_task(mul_task);
    job.add_task(fact_task);
    job.set_finish_callback([&](Job*){
        sum = *((int*)(sum_task->get_result().get_data()));
...
TEST(test_job, base_dependency_tasks) {
    ...
    job.add_task(mul_task);
    job.add_task(fact_task);
    job.add_task(mean_task);
    
    job.add_dependency(mean_task, sum_task);
    job.add_dependency(mean_task, mul_task);
    job.add_dependency(mean_task, fact_task);
